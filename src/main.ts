...
function shell(content: string) {
  return `${demoMode ? `<aside class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved</strong><span><button id="reset-demo" type="button">Reset demo</button><button id="start-real" type="button">Start for real</button></span></aside>` : ''}
    <header class="site-header"><a class="brand" href="/" data-route aria-label="Takeout Tidy home"><span class="brand-mark" aria-hidden="true">${icon('photo')}</span><span>Takeout Tidy</span></a><nav aria-label="Primary navigation"><a href="/demo" data-route>Demo</a><a href="/#how-it-works">How repair works</a><a href="/privacy/" data-route>Privacy</a></nav></header>
    ${content}
...
async function downloadSettings() { const blob = new Blob([await exportPreferences()], { type: 'application/json' }); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'takeout-tidy-settings.json'; link.click(); URL.revokeObjectURL(link.href); }
